...
#include <mpif.h>
   ...
   type(mct_gsMap)  :: gsMap_aa(num_inst_atm)    ! on component pes
   type(mct_gsMap)  :: gsMap_ll(num_inst_lnd)
   type(mct_gsMap)  :: gsMap_oo(num_inst_ocn)
   type(mct_gsMap)  :: gsMap_ii(num_inst_ice)
   type(mct_gsMap)  :: gsMap_rr(num_inst_rof)
   type(mct_gsMap)  :: gsMap_gg(num_inst_glc)
   type(mct_gsMap)  :: gsMap_ss(num_inst_lnd)
...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   integer  :: index_r2o_Forr_roff
   integer  :: index_r2o_Forr_ioff
   integer  :: index_l2x_Flrl_rofliq
   integer  :: index_l2x_Flrl_rofice
   integer  :: index_l2r_Flrl_rofliq
   integer  :: index_l2r_Flrl_rofice
   ...
   integer, pointer :: atm_petlist(:), lnd_petlist(:), ice_petlist(:), ocn_petlist(:), &
                       glc_petlist(:), cpl_petlist(:), rof_petlist(:)
   ...
   integer :: eai, eli, eoi, eii, egi, eri, exi, efi  ! component instance counters
...
subroutine ccsm_init()
 ...
 106  format( A, f23.12)
   ...
   call seq_timemgr_clockInit(seq_SyncClock,nlfilename,read_restart,rest_file,mpicom_gloid, &
        EClock_d, EClock_a, EClock_l, EClock_o, EClock_i, Eclock_g, Eclock_r)
   if (iamroot_CPLID) then
   ...
   if (iamroot_CPLID )then
      write(logunit,*) ' '
      write(logunit,F00) 'Initialize each component: atm, lnd, rof, ocn, ice, and glc'
   ...
   do eai = 1,num_inst_atm
      if (iamroot_CPLID) write(logunit,F00) 'Initialize atm component '//trim(atm_name(eai))
      call shr_sys_flush(logunit)
   ...
   do eli = 1,num_inst_lnd
      if (iamroot_CPLID) write(logunit,F00) 'Initialize lnd component '//trim(lnd_name(eli))
      call shr_sys_flush(logunit)
   ...
   do eri = 1,num_inst_rof
      if (iamroot_CPLID) write(logunit,F00) 'Initialize rof component '//trim(rof_name(eri))
      call shr_sys_flush(logunit)
   ...
   do eoi = 1,num_inst_ocn
      if (iamroot_CPLID) write(logunit,F00) 'Initialize ocn component '//trim(ocn_name(eoi))
      call shr_sys_flush(logunit)
   ...
   do eii = 1,num_inst_ice
      if (iamroot_CPLID) write(logunit,F00) 'Initialize ice component '//trim(ice_name(eii))
      call shr_sys_flush(logunit)
   ...
   do egi = 1,num_inst_glc
      if (iamroot_CPLID) write(logunit,F00) 'Initialize glc component '//trim(glc_name(egi))
      call shr_sys_flush(logunit)
...
!   endif
      ...
      do eai = 1,num_inst_atm
         ...
         if (iamin_CPLATMID(eai)) then
            ...
            if (eai == 1) then  ! create dom_ax
               ...
               call seq_map_map(mapper_Ca2x(eai), dom_aa(eai)%data, dom_ax%data)
               ...
               call seq_map_map(mapper_Ca2x(eai), dom_aa(eai)%data, dom_tmp%data)
            ...
            call seq_map_map(mapper_Ca2x(eai), x2a_aa(eai), x2a_ax(eai))
        endif
     enddo
   endif
      ...
      do eli = 1,num_inst_lnd
         ...
         if (iamin_CPLLNDID(eli)) then
            ...
            if (eli == 1) then
               ...
               call seq_map_map(mapper_Cl2x(eli), dom_ll(eli)%data, dom_lx%data)
            else
               ...
               call seq_map_map(mapper_Cl2x(eli), dom_ll(eli)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_lx,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2l_ll(eli))
            call seq_map_map(mapper_Cl2x(eli), x2l_ll(eli), x2l_lx(eli))
         endif
      enddo
   end if
      ...
      do eli = 1,num_inst_lnd
         ...
         if (iamin_CPLLNDID(eli)) then
            ...
            if (eli == 1) then
               ...
               call seq_map_map(mapper_Cs2x(eli), dom_ss(eli)%data, dom_sx%data)
            else
               ...
               call seq_map_map(mapper_Cs2x(eli), dom_ss(eli)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_sx,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2s_ss(eli))
            call seq_map_map(mapper_Cs2x(eli), x2s_ss(eli), x2s_sx(eli))
         endif
      enddo
   end if
      ...
      do eri = 1,num_inst_rof
         ...
         if (iamin_CPLROFID(eri)) then
            ...
            if (eri == 1) then
               ...
               call seq_map_map(mapper_Cr2x(eri), dom_rr(eri)%data, dom_rx%data)
            else
               ...
               call seq_map_map(mapper_Cr2x(eri), dom_rr(eri)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_rx,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2r_rr(eri))
            call seq_map_map(mapper_Cr2x(eri), x2r_rr(eri), x2r_rx(eri))
         endif
      enddo
   end if
      ...
      do eii = 1,num_inst_ice
         ...
         if (iamin_CPLICEID(eii)) then
            ...
            if (eii == 1) then
               ...
               call seq_map_map(mapper_Ci2x(eii), dom_ii(eii)%data, dom_ix%data)
            else
               ...
               call seq_map_map(mapper_Ci2x(eii), dom_ii(eii)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_ix,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2i_ii(eii))
            call seq_map_map(mapper_Ci2x(eii), x2i_ii(eii), x2i_ix(eii))
         endif
      enddo
   endif
      ...
      do egi = 1,num_inst_glc
         ...
         if (iamin_CPLGLCID(egi)) then
            ...
            if (egi == 1) then
               ...
               call seq_map_map(mapper_Cg2x(egi), dom_gg(egi)%data, dom_gx%data)
            else
               ...
               call seq_map_map(mapper_Cg2x(egi), dom_gg(egi)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_gx,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2g_gg(egi))
            call seq_map_map(mapper_Cg2x(egi), x2g_gg(egi), x2g_gx(egi))
         endif
      enddo
   endif
      ...
      do eoi = 1,num_inst_ocn
         ...
         if (iamin_CPLOCNID(eoi)) then
            ...
            if (eoi == 1) then
               ...
               call seq_map_map(mapper_Co2x(eoi), dom_oo(eoi)%data, dom_ox%data)
            else
               ...
               call seq_map_map(mapper_Co2x(eoi), dom_oo(eoi)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_ox,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_zero(x2o_oo(eoi))
            call seq_map_map(mapper_Co2x(eoi), x2o_oo(eoi), x2o_ox(eoi))
   ...
   if (iamin_CPLID) then
      ...
      do eli = 1,num_inst_lnd
         ...
         call mct_avect_init(l2racc_lx(eli), rlist=seq_flds_l2r_fields, lsize=lsize_l)
         call mct_aVect_zero(l2racc_lx(eli))
      enddo
      l2racc_lx_cnt = 0
      call mct_avect_init(l2r_rx, rList=seq_flds_l2r_fields, lsize=lsize_r)
      call mct_avect_zero(l2r_rx)

      do eri = 1,num_inst_rof
         call mct_aVect_init(r2o_ox(eri), rList=seq_flds_r2o_fields, lsize=lsize_o)
         call mct_aVect_zero(r2o_ox(eri))
         call mct_aVect_init(r2x_lx(eri), rlist=seq_flds_r2x_fields, lsize=lsize_l)
         call mct_aVect_zero(r2x_lx(eri)) 
         call mct_avect_init(r2oacc_rx(eri), rList=seq_flds_r2o_fields, lsize=lsize_r)
         call mct_aVect_zero(r2oacc_rx(eri))
      enddo
      r2oacc_rx_cnt = 0
      ...
      if (rof_present) then
         ...
         index_r2o_Forr_roff = mct_avect_indexRA(r2oacc_rx(ens1),"Forr_roff",perrWith='indexb Forr_roff')
         index_r2o_Forr_ioff = mct_avect_indexRA(r2oacc_rx(ens1),"Forr_ioff",perrWith='indexb Forr_ioff')
      endif
      if (lnd_present) then
         ...
         index_l2r_Flrl_rofliq = mct_avect_indexRA(l2racc_lx(ens1),"Flrl_rofliq",perrWith='indexa Flrl_rofliq')
         index_l2r_Flrl_rofice = mct_avect_indexRA(l2racc_lx(ens1),"Flrl_rofice",perrWith='indexa Flrl_rofice')
      endif
   endif
   ...
   if (iamin_CPLID) then
      ...
      if (ocn_present) then
         ...
         call seq_map_init_rcfile(mapper_Sa2o,gsmap_ax,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','atm2ocnSmapname:','atm2ocnSmaptype:',samegrid_ao, &
         ...
         call seq_map_init_rcfile(mapper_Va2o,gsmap_ax,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','atm2ocnVmapname:','atm2ocnVmaptype:',samegrid_ao, &
         ...
         call seq_map_init_rcfile(mapper_Fa2o,gsmap_ax,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','atm2ocnFmapname:','atm2ocnFmaptype:',samegrid_ao, &
         ...
         call seq_map_init_rcfile(mapper_So2a,gsmap_ox,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','ocn2atmSmapname:','ocn2atmSmaptype:',samegrid_ao, &
         ...
         call seq_map_init_rcfile(mapper_Fo2a,gsmap_ox,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','ocn2atmFmapname:','ocn2atmFmaptype:',samegrid_ao, &
         ...
         if (samegrid_ao) then
            ...
            call seq_map_readdata('seq_maps.rc','ocn2atmFmapname:',mpicom_CPLID, CPLID, &
      ...
      if (ice_present) then
         ...
         call seq_map_init_rcfile(mapper_Si2a,gsmap_ix,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','ice2atmSmapname:','ice2atmSmaptype:',samegrid_ao, &
         ...
         call seq_map_init_rcfile(mapper_Fi2a,gsmap_ix,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','ice2atmFmapname:','ice2atmFmaptype:',samegrid_ao, &
      ...
      if (rof_present .and. ocnrof_prognostic) then
         ...
         call seq_map_init_rcfile(mapper_Fr2o,gsmap_rx,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','rof2ocnFmapname:','rof2ocnFmaptype:',samegrid_ro, &
             'mapper_Fr2o initialization',esmf_map_flag)
         if (.not.samegrid_ro) then
            call seq_map_readdata('seq_maps.rc','rof2ocnFmapname:',mpicom_CPLID, CPLID, &
      ...
      if (rof_present .and. lnd_present ) then ! TODO - land and rof might be on the same grid (see below)
         if (rof_prognostic) then 
            ...
            call seq_map_init_rcfile( mapper_Fl2r,gsmap_lx,gsmap_rx,mpicom_CPLID, &
                 'seq_maps.rc','lnd2rofFmapname:','lnd2rofFmaptype:',samegrid=.false., &
         ...
         if (flood_present .and. lnd_prognostic) then
            ...
            call seq_map_init_rcfile( mapper_Fr2l,gsmap_rx,gsmap_lx,mpicom_CPLID, &
                 'seq_maps.rc','rof2lndFmapname:', 'rof2lndFmaptype:',samegrid=.false., &
      ...
      if (lnd_present) then
         ...
         call seq_map_init_rcfile(mapper_Sa2l,gsmap_ax,gsmap_lx,mpicom_CPLID, &
             'seq_maps.rc','atm2lndSmapname:','atm2lndSmaptype:',samegrid_al, &
         ...
         call seq_map_init_rcfile(mapper_Fa2l,gsmap_ax,gsmap_lx,mpicom_CPLID, &
             'seq_maps.rc','atm2lndFmapname:','atm2lndFmaptype:',samegrid_al, &
         ...
         call seq_map_init_rcfile(mapper_Sl2a,gsmap_lx,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','lnd2atmSmapname:','lnd2atmSmaptype:',samegrid_al, &
         ...
         call seq_map_init_rcfile(mapper_Fl2a,gsmap_lx,gsmap_ax,mpicom_CPLID, &
             'seq_maps.rc','lnd2atmFmapname:','lnd2atmFmaptype:',samegrid_al, &
             'mapper_Fl2a initialization',esmf_map_flag)
         if (samegrid_al) then
            call seq_map_map(mapper_Sa2l, dom_ax%data, dom_lx%data, fldlist='aream')
         else
            call seq_map_readdata('seq_maps.rc','atm2lndFmapname:',mpicom_CPLID, CPLID, &
   ...
   if (atm_present) then
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            call seq_map_map(mapper_Cx2a(eai), dom_ax%data, dom_aa(eai)%data)
            if (iamin_ATMID(eai)) then
            ...
            call seq_map_map(mapper_Ca2x(eai), a2x_aa(eai), a2x_ax(eai))
         endif
      enddo
   endif
   ...
   if (lnd_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            call seq_map_map(mapper_Cx2l(eli), dom_lx%data, dom_ll(eli)%data)
            if (iamin_LNDID(eli)) then
            ...
            call seq_map_map(mapper_Cl2x(eli), l2x_ll(eli), l2x_lx(eli))
         endif
      enddo
   end if
   ...
   if (sno_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            call seq_map_map(mapper_Cx2s(eli), dom_sx%data, dom_ss(eli)%data)
            if (iamin_LNDID(eli)) then
            ...
            call seq_map_map(mapper_Cs2x(eli), s2x_ss(eli), s2x_sx(eli))
         endif
      enddo
   end if
   ...
   if (rof_present) then
      do eri = 1,num_inst_rof
         if (iamin_CPLROFID(eri)) then
            call seq_map_map(mapper_Cx2r(eri), dom_rx%data, dom_rr(eri)%data)
            if (iamin_ROFID(eri)) then
            ...
            call seq_map_map(mapper_Cr2x(eri), r2x_rr(eri), r2x_rx(eri))
         endif
      enddo
   end if
   ...
   if (ocn_present) then
      do eoi = 1,num_inst_ocn
         if (iamin_CPLOCNID(eoi)) then
            call seq_map_map(mapper_Cx2o(eoi), dom_ox%data, dom_oo(eoi)%data)
            if (iamin_OCNID(eoi)) then
           ...
           call seq_map_map(mapper_Co2x(eoi), o2x_oo(eoi), o2x_ox(eoi))
        endif
      enddo
   endif
   ...
   if (ice_present) then
      do eii = 1,num_inst_ice
         if (iamin_CPLICEID(eii)) then
            call seq_map_map(mapper_Cx2i(eii), dom_ix%data, dom_ii(eii)%data)
            if (iamin_ICEID(eii)) then
            ...
            call seq_map_map(mapper_Ci2x(eii), i2x_ii(eii), i2x_ix(eii))
         endif
      enddo
   endif
   ...
   if (glc_present) then
      do egi = 1,num_inst_glc
         if (iamin_CPLGLCID(egi)) then
            call seq_map_map(mapper_Cx2g(egi), dom_gx%data, dom_gg(egi)%data)
            if (iamin_GLCID(egi)) then
            ...
            call seq_map_map(mapper_Cg2x(egi), g2x_gg(egi), g2x_gx(egi))
         endif
      enddo
   endif
   ...
   if (iamin_CPLID) then
      ...
      do efi = 1,num_inst_frc
         ...
         call seq_frac_set(i2x_ix(eii), &
                        cdata_ax, cdata_ix, cdata_lx, cdata_ox, cdata_gx, cdata_rx, &
                        ice_present, ocn_present, lnd_present, glc_present, rof_present, &
                        fractions_ax(efi), fractions_ix(efi), fractions_lx(efi), &
                        fractions_ox(efi), fractions_gx(efi), fractions_rx(efi), &
...
!tcx is this correct? relation between xao and frc for ifrad and ofrad
   ...
   if (atm_prognostic) then
      ...
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            call seq_map_map(mapper_Cx2a(eai), x2a_ax(eai), x2a_aa(eai))
         endif
      enddo
      if (iamin_CPLALLATMID) then
         call seq_infodata_exchange(infodata,CPLALLATMID,'cpl2atm_init')
 ...
 107  format( A, 2i8, A, f12.4, A, f12.4 )
   ...
   do while ( .not. stop_alarm)
      ...
      if (ocn_present .and. ocnrun_alarm) then
         ...
         !----------------------------------------------------
         ! ocn prep
         ! note due to x2oacc and r2oacc, need to be careful filling x2o_oo
         ! average x2oacc and r2oacc first, separately, now averages in those AVs
         ! map r2oacc_rx to r2o_ox then copy r2o_ox into x2oacc_ox
...
!              call mct_aVect_average(x2oacc_ox(eoi))
            ...
            if (rof_present .and. ocnrof_prognostic) then
               ...
               if (r2oacc_rx_cnt > 0) then
                  call t_drvstartf ('driver_ocnprep_ravg',barrier=mpicom_CPLID)
                  do eri = 1,num_inst_rof
                     r2oacc_rx(eri)%rAttr = r2oacc_rx(eri)%rAttr / (r2oacc_rx_cnt*1.0_r8)
                  enddo
                  r2oacc_rx_cnt = 0
                  ...
                  do eri = 1,num_inst_rof
                     call seq_map_map(mapper_Fr2o, r2oacc_rx(eri), r2o_ox(eri),norm=.false.)
                  enddo
                  if (do_hist_r2x) then
                     do eri = 1,num_inst_rof
                        call seq_hist_writeaux(EClock_d,'r2oacc'//trim(lnd_suffix(eri)), &
                                'domr',cdata_rx, r2oacc_rx(eri), rof_nx,rof_ny,1)
                     enddo
                  endif
                  ...
                  do eoi = 1,num_inst_ocn
                     ...
                     ! from r2o_ox to x2oacc_ox
                     call mct_aVect_copy(aVin=r2o_ox(eri), aVout=x2oacc_ox(eoi))
                     call mct_aVect_copy(aVin=r2o_ox(eri), aVout=x2o_ox(eoi))   ! for history file
                  enddo
         ...
         if (iamin_CPLALLOCNID .and. ocn_prognostic) then
            call t_drvstartf ('DRIVER_C2O',barrier=mpicom_CPLALLOCNID)
            do eoi = 1,num_inst_ocn
               if (iamin_CPLOCNID(eoi)) then
                  call t_drvstartf ('driver_c2o_ocnx2ocno',barrier=mpicom_CPLOCNID(eoi))
                  call seq_map_map(mapper_Cx2o(eoi), x2oacc_ox(eoi), x2o_oo(eoi))
...
!tcx needs work
               if (flood_present) then
                  ...
                  do eri = 1,num_inst_rof
                     ...
                     call shr_sys_abort('tcx ccsm_comp work in progress map Fr2l')
!tcx??                     call seq_map_map(mapper_Fr2l, r2x_rx(eri), r2x_lx(eri), &
!                          fldlist=seq_flds_r2x_fluxes, norm=.true., &
!                          avwts_s=fractions_rx(efi), avwtsfld_s='lfrinsc')
                  end do
...
!--------------
         ...
         if (iamin_CPLALLLNDID) then
            call t_drvstartf ('DRIVER_C2L', barrier=mpicom_CPLALLLNDID)
            do eli = 1,num_inst_lnd
               if (iamin_CPLLNDID(eli)) then
                  if (lnd_prognostic) then
                     call t_drvstartf ('driver_c2l_lndx2lndl', &
                                       barrier=mpicom_CPLLNDID(eli))
                     call seq_map_map(mapper_Cx2l(eli), x2l_lx(eli), x2l_ll(eli))
                  ...
                  if (glc_present .and. sno_prognostic) then
                     call t_drvstartf ('driver_c2l_snox2snos', &
                                       barrier=mpicom_CPLLNDID(eli))
                     call seq_map_map(mapper_Cx2s(eli), x2s_sx(eli), x2s_ss(eli))
...
!tcx fails     call map_atm2ice_mct( cdata_ax, a2x_ax(eai), cdata_ix, a2x_ix(eai) )
         ...
         if (iamin_CPLALLICEID .and. ice_prognostic) then
            call t_drvstartf ('DRIVER_C2I',barrier=mpicom_CPLALLICEID)
            do eii = 1,num_inst_ice
               if (iamin_CPLICEID(eii)) then
                  call t_drvstartf ('driver_c2i_icex2icei',barrier=mpicom_CPLICEID(eii))
                  call seq_map_map(mapper_Cx2i(eii), x2i_ix(eii), x2i_ii(eii))
      ...
      if (rof_present .and. rofrun_alarm) then
         ...
         if (iamin_CPLID .and. rof_prognostic) then
            ...
            do eri = 1,num_inst_rof
               ...
               if (l2racc_lx_cnt > 0) then
                  l2racc_lx(eli)%rAttr = l2racc_lx(eli)%rAttr / (l2racc_lx_cnt*1.0_r8)
               endif
               ...
               call seq_map_map(mapper_Fl2r, l2racc_lx(eli), l2r_rx, &
                  fldlist=seq_flds_l2r_fluxes, norm=.true., &
                  avwts_s=fractions_lx(efi), avwtsfld_s='lfrin')
         ...
         if (iamin_CPLALLROFID .and. rof_prognostic) then
            call t_drvstartf ('DRIVER_C2R', barrier=mpicom_CPLALLLNDID)
            do eri = 1,num_inst_rof
               if (iamin_CPLROFID(eri)) then
                  call t_drvstartf ('driver_c2r_rofx2rofi',barrier=mpicom_CPLROFID(eri))
                  call seq_map_map(mapper_Cx2r(eri), x2r_rx(eri), x2r_rr(eri))
                  call t_drvstopf  ('driver_c2r_rofx2rofi')
               end if
            end do
            call t_drvstopf  ('DRIVER_C2R')
         end if
      ...
      if (rof_present .and. rofrun_alarm) then
         do eri = 1,num_inst_rof
            if (iamin_ROFID(eri)) then
               ...
               call t_drvstartf  ('DRIVER_ROF_RUN')
               if (drv_threading) call seq_comm_setnthreads(nthreads_ROFID)
               ...
               if (run_barriers) then
                  ...
                  cktime_acc(3) = cktime_acc(3) + cktime
                  cktime_cnt(3) = cktime_cnt(3) + 1
                  write(logunit,107) ' rstamp rof_time: model date = ', &
                       ymd,tod,' avg dt = ',cktime_acc(3)/cktime_cnt(3),' dt = ',cktime
               endif
            end if
         enddo
      ...
      if (ocn_present .and. ocnnext_alarm) then
         call t_drvstartf ('DRIVER_O2CT',barrier=mpicom_CPLALLOCNID)
         do eoi = 1,num_inst_ocn
            if (iamin_CPLOCNID(eoi)) then
      ...
      if (ocn_present .and. iamin_CPLID) then
         ...
         if (ocn_prognostic) then
            ...
            do eoi = 1,num_inst_ocn
               ...
               efi = mod((eoi-1),num_inst_frc) + 1
               call mrg_x2o_run_mct( cdata_ox, a2x_ox(eai), i2x_ox(eii), xao_ox(exi), fractions_ox(efi), x2o_ox(eoi) )
            enddo
...
!     !        call mct_aVect_accumulate(x2o_ox(eoi), x2oacc_ox(eoi))
      ...
      if (iamin_CPLALLLNDID) then
         ...
         do eli = 1,num_inst_lnd
            call t_drvstartf ('DRIVER_L2C', barrier=mpicom_CPLALLLNDID)
            if (iamin_CPLLNDID(eli)) then
               if (lnd_present) then
      ...
      if (iamin_CPLID) then
         do eli = 1,num_inst_lnd
            if (l2racc_lx_cnt == 0) then
               l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofliq,:) = l2x_lx(eli)%rAttr(index_l2x_Flrl_rofliq,:)
               l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofice,:) = l2x_lx(eli)%rAttr(index_l2x_Flrl_rofice,:)
            else
               l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofliq,:) = l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofliq,:) + &
                                                               l2x_lx(eli)%rAttr(index_l2x_Flrl_rofliq,:)
               l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofice,:) = l2racc_lx(eli)%rAttr(index_l2r_Flrl_rofice,:) + &
      ...
      if (sno_present .and. glcrun_alarm) then
         ...
         if (iamin_CPLALLGLCID .and. glc_prognostic) then
            call t_drvstartf ('DRIVER_C2G',barrier=mpicom_CPLALLGLCID)
            do egi = 1,num_inst_glc
               if (iamin_CPLGLCID(egi)) then
                  call t_drvstartf ('driver_c2g_glcx2glcg',barrier=mpicom_CPLGLCID(egi))
                  call seq_map_map(mapper_Cx2g(egi), x2g_gx(egi), x2g_gg(egi))
      ...
      if (rof_present .and. rofrun_alarm) then
      if (iamin_CPLALLROFID) then
         call t_drvstartf ('DRIVER_R2C', barrier=mpicom_CPLALLROFID)
         do eri = 1,num_inst_rof
            if (iamin_CPLROFID(eri)) then
         ...
         if (iamin_CPLID) then
            ...
            if (ocnrof_prognostic) then
               ...
               do eri = 1,num_inst_rof
                  if (r2oacc_rx_cnt == 0) then
                     r2oacc_rx(eri)%rAttr(index_r2o_Forr_roff,:) = r2x_rx(eri)%rAttr(index_r2x_Forr_roff,:) * flux_epbalfact
                     r2oacc_rx(eri)%rAttr(index_r2o_Forr_ioff,:) = r2x_rx(eri)%rAttr(index_r2x_Forr_ioff,:) * flux_epbalfact
                  else
                     r2oacc_rx(eri)%rAttr(index_r2o_Forr_roff,:) = r2oacc_rx(eri)%rAttr(index_r2o_Forr_roff,:) + &
                        r2x_rx(eri)%rAttr(index_r2x_Forr_roff,:) * flux_epbalfact
                     r2oacc_rx(eri)%rAttr(index_r2o_Forr_ioff,:) = r2oacc_rx(eri)%rAttr(index_r2o_Forr_ioff,:) + &
                        r2x_rx(eri)%rAttr(index_r2x_Forr_ioff,:) * flux_epbalfact
                  endif
               enddo
               r2oacc_rx_cnt = r2oacc_rx_cnt + 1
      ...
      ! it will also use the current r2o_ox here which is the value from the last timestep
      ...
      if (iamin_CPLID .and. do_budgets) then
         ...
         if (ocn_present) then
            call seq_diag_ocn_mct(dom_ox, fractions_ox(ens1), o2x_o=o2x_ox(ens1), x2o_o=x2o_ox(ens1), &
                 xao_o=xao_ox(ens1), r2x_o=r2o_ox(ens1))
         endif
         if (ice_present) then
      ...
      if (ice_present .and. icerun_alarm) then
      if (iamin_CPLALLICEID) then
         call t_drvstartf ('DRIVER_I2C',barrier=mpicom_CPLALLICEID)
         do eii = 1,num_inst_ice
            if (iamin_CPLICEID(eii)) then
...
!            call seq_hist_spewav('x2a_ax(eai)',gsmap_ax,x2a_ax(eai),atm_nx,atm_ny,1)
         ...
         if (iamin_CPLALLATMID) then
         if (atm_prognostic) then
            call t_drvstartf ('DRIVER_C2A',barrier=mpicom_CPLALLATMID)
            do eai = 1,num_inst_atm
               if (iamin_CPLATMID(eai)) then
      ...
      if (atm_present .and. atmrun_alarm) then
         ! -----
         ! special import loop for sequencial instances
         ! -----
         call seq_infodata_putData(infodata,atm_phase=0)
         do eai = 1,num_inst_atm
            if (iamin_ATMID(eai)) then
               call t_drvstartf ('DRIVER_ATM_RUN_IMP',barrier=mpicom_ATMID(eai))
               if (drv_threading) call seq_comm_setnthreads(nthreads_ATMID)
               ...
               if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
               call t_drvstopf  ('DRIVER_ATM_RUN_IMP')
            endif
         enddo
         call seq_infodata_putData(infodata,atm_phase=1)

         ! -----
         ! standard run loop, unchanged
         ! -----
         do eai = 1,num_inst_atm
            if (iamin_ATMID(eai)) then
               if (run_barriers) then
      ...
      if (glc_present .and. glcrun_alarm) then
      if (iamin_CPLALLGLCID) then
         call t_drvstartf ('DRIVER_G2C',barrier=mpicom_CPLALLGLCID)
         do egi = 1,num_inst_glc
            if (iamin_CPLGLCID(egi)) then
      ...
      if (atm_present .and. atmrun_alarm) then
      if (iamin_CPLALLATMID) then
         call t_drvstartf ('DRIVER_A2C',barrier=mpicom_CPLALLATMID)
         do eai = 1,num_inst_atm
            if (iamin_CPLATMID(eai)) then
      ...
      if (iamin_CPLALLOCNID) then
      if (ocn_present .and. ocnnext_alarm) then
         call t_drvstartf ('DRIVER_O2C',barrier=mpicom_CPLALLOCNID)
         do eoi = 1,num_inst_ocn
            if (iamin_CPLOCNID(eoi)) then
      ...
      if (tod == 0 .or. info_debug > 1) then
         ...
         if (iamroot_CPLID .or. iamroot_OCNID(ens1) .or. iamroot_ATMID(ens1) .or. &
             iamroot_LNDID(ens1) .or. iamroot_ICEID(ens1) .or. iamroot_GLCID(ens1)) then
...
subroutine ccsm_comp_init(comp, importState, exportState, clock, rc)
   ...
   integer             :: localrc
   type(ESMF_GridComp) :: atmComp, lndComp, iceComp, ocnComp, glcComp, rofComp, cplComp
   type(ESMF_VM)       :: vm
