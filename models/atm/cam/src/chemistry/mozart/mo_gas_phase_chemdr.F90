module mo_gas_phase_chemdr
  ...
  integer :: o3_ndx
  integer :: het1_ndx
  integer :: ndx_cldfr, ndx_cmfdqr, ndx_nevapr, ndx_cldtop, ndx_pblh, ndx_prain
...
!
    ...
    !-----------------------------------------------------------------------  
    if ( trim(chem_name) == 'super_fast_llnl' .or. trim(chem_name) == 'super_fast_llnl_mam3' ) then
    ...
    ! aerosol packages
    !
    if(.not. use_SPCAM) then  ! regular CAM
#if (defined MODAL_AERO)
    dvmrdt_sv1 = vmr
    dvmrcwdt_sv1 = vmrcw
#endif
