...
subroutine modal_aero_wateruptake_dr(state, pbuf)
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   do m = 1, nmodes
      ...
      ! get mode properties
      call rad_cnst_get_mode_props(0, m, sigmag=sigmag,  &
      ...
      do ii = 1, crm_nx
         ...
         do k = 1, pver
            do i = 1, ncol
               ...
               rh(i,k) = h2ommr(i,k)/qsat_water(t(i,k),pmid(i,k))
               ...
               if(use_SPCAM) then
                 ...
                 if(mm.le.crm_nz) then
                   rh_crm(i, ii, jj, k) = h2ommr_crm(i,ii,jj,mm)/qsat_water(t_crm(i, ii, jj, mm), pmid(i,k))
