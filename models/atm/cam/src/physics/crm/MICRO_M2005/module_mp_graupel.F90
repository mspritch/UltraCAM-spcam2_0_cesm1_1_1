...
!bloss: This is version 1.4 of Hugh Morrison's two moment, five class scheme.

! hm, modified microphysics 8/31/11 for consistency with V3 microphysics
! Current version is tagged as V3
...
!                 PRC(K)=min(1.,100./dx)**2*1350.*QC3D(K)**2.47*  &    !hparish followed Marat's email in this fudge factor correction. compare against the next line.
...
!                NPRC1(K) = MIN(NPRC1(K),NPRC(K))        !hparish followed a bug fix in SAMUW, done by hm. 
...
!            NPRACG(K)=NPRACG(K)-DUM            !hparish followed SAMUW. compare against the line above.
...
!          DUM3=(-QVS(K)*RHO(K)/(PRES(K)-EVS(K))+DQSDT/CP)*G*DUM     !hparish followed SAMUW bug fixes.
