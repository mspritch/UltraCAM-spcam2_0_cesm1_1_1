...
use micro_vary, only : update_micro_vary_vals,vtice
#endif
contains
...
#endif
#ifdef MICRO_VARY
                       ,microvary_qci0,microvary_qcw0,microvary_vtice,microvary_tbgmin,microvary_tbgmax &
#endif
...
!---------------------------------------------------------------
	...
	use crm_grid_class, only: gr   ! Variable
    use clubb_precision, only: core_rknd ! Constants
        ...
        use extra,    only: EUC_NORM     !hparish added for debugging crm_acc
...
#endif
#ifdef MICRO_VARY
         real(r8), intent(out) :: microvary_qci0, microvary_qcw0,microvary_vtice, microvary_tbgmin,microvary_tbgmax
#endif
!  Input/Output:
#ifdef CLUBB_CRM
...
!  Output
         ...
         real(r8), intent(out) :: utendout(plev) ! tendency of u for outputting
         real(r8), intent(out) :: vtendout(plev) ! tendency of v for outputting
...
#endif
#ifdef CRMACCEL
         logical, intent (in)  :: use_crm_accel
         real(r8), intent(in)  :: crm_accel_factor
         logical :: crm_accel_ceaseflag,accel_skipping
         integer :: accel_skip_step
#endif

!  Local space:
! hparish modified the following real data types to double precision for
! variables relevant to crm mean acceleration.
! This is critical for crm acceleration at small dt.
! The requirement for each and every variable to be double is not tested
! individually. Might have room for improvement by switching "some of them" back
! to single precison for cost purposes.
! See hparish comments in acceleration routine for more detail.
! date: 8/29/2015.

        real dummy(nz)
        real(r8) t00(nz)
        real(r8) l2norm_t,l2norm_qv,l2norm_mf,l2norm_cl,l2norm_qcl  !hparish added for debugging
        real fluxbtmp(nx,ny), fluxttmp(nx,ny) !bloss
        real(r8) tln(plev), qln(plev), qccln(plev), qiiln(plev)
        real uln(plev), vln(plev)
...
real(kind=core_rknd), dimension(nzm) :: &
  rtm_column ! Total water (vapor + liquid)     [kg/kg]
        ...
        real ntotal_step
#ifdef CRMSUBS
        integer :: k1,k2,l1,l2,n
        real(r8) :: t_tend (nx,ny,nzm)
        real(r8) :: q_tend (nx,ny,nzm)
        real(r8) :: dq,t_vtend,q_vtend
#endif

#ifdef CRMTRANSLATE
        real(r8) :: mass_sum,u_remove,v_remove
#endif
...
!-----------------------------------------------
        ...
        !  dostatis = .false.    ! statistics are now enabled --  mwyant 
        idt_gl = 1._r8/dt_gl
        ptop = plev-nzm+1
...
#ifdef STRATOKILLER
        stratokill_debug_flag = 0
       if (latitude0 .gt. -72. .and. latitude0 .lt. -68. .and. longitude0 .gt. 103. .and. longitude0 .lt. 107.) then
!         stratokill_debug_flag = 1
       endif
#endif
        pi = acos(-1.)
...
!        write (6,*) 'HEY task ', iam,' called update_micro with step=',igstep
        ...
        microvary_vtice = vtice
        microvary_tbgmin = tbgmin
        microvary_tbgmax = tbgmax
...
#ifdef LESVISCOSITY120
           grdf_x(k) = min(120.,dx**2/(adz(k)*dz)**2)
           grdf_y(k) = min(120.,dy**2/(adz(k)*dz)**2)
#endif
           grdf_z(k) = 1.
        end do
...
!BSINGH(04/23/2015): We are going to use the logoc for CLUBB_CRM as default
! to avoid rhow(nz)=0 situations
...
!#else 
!        rhow(nz)= 2*rhow(nzm) - rhow(nzm-1)
!#endif /*CLUBB_CRM*/!Commented out by BALLI  
        colprec=0
        colprecs=0
...
!      if ( pmid(l)/100. .ge. 850. .and. pmid(l)/100. .lt. 950. ) then

!       do j=1,ny
!         do i=1,nx
!           u_mean(k) = u_mean(k) + u_crm(i,j,k)
!           v_mean(k) = v_mean(k) + v_crm(i,j,k)
!         end do
!       end do
!       u_mean(k) = u_mean(k) / float(nx*ny)
!       v_mean(k) = v_mean(k) / float(nx*ny)

!      endif   !cloud layer
 
!     do j=1,ny
!       do i=1,nx
!         u_crm(i,j,1:nzm) = u_crm(i,j,1:nzm) - u_mean(1:nzm)
!         v_crm(i,j,1:nzm) = v_crm(i,j,1:nzm) - v_mean(1:nzm)
!       end do
!     end do
!
!     end do
...
! now do the same thing to dudt and dvdt if nstep is nonzero. where? don't know yet.
...
!---mhwang
 ...
 ! -dschanen UWM 11 Feb 2010
     ...
     u_remove = 0.
     v_remove = 0.
     if (landfrac .eq. 0. .and. abs(latitude0) .le. 35.) then
       do k=1,nzm
         l = plev-k+1
         if (pmid(l)/100. .ge. 850. .and. pmid(l)/100. .le. 900.) then
              u_remove = u_remove + ul(l)*pdel(l)
              v_remove = v_remove + vl(l)*pdel(l)
              mass_sum = mass_sum + pdel(l)
         endif
       end do
     endif
     if (mass_sum .gt. 0.) then
       u_remove = u_remove/mass_sum
     ...
     do k=1,nzm
       l=plev-k+1
       uln(l) = min( umax, max(-umax,uln(l)) )
       vln(l) = min( umax, max(-umax,vln(l)) )*YES3D
#else
...
! HP implements the advection mods bellow following Mike's UW implementation.
...
! we will work in k space.
! therefore (i) dpres ~ pres(k+1)-pres(k) = pmid(l-1)-pmid(l)
! and (ii) dpres ~ pres(k) - pres(k-1) = pmid(l)-pmid(l+1) 
! RHS form should apply to all l-type variables.

! For this clause we are in case (i)
          qtend(k) = - omegal(l)*( ql(l-1)+qccl(l-1)+qiil(l-1)-ql(l)-qccl(l)-qiil(l) )/(pmid(l-1)-pmid(l))
          ttend(k) = - omegal(l)*((tl(l-1)+gamaz(k+1)-fac_cond*(qccl(l-1)+qiil(l-1))-fac_fus*qiil(l-1)) - &
                    (tl(l)+gamaz(k)-fac_cond*(qccl(l)+qiil(l))-fac_fus*qiil(l)) )/(pmid(l-1)-pmid(l))            
        else
! For this clause we are in case 2
! and (ii) dpres ~ pres(k) - pres(k-1) = pmid(l)-pmid(l+1)           
          qtend(k) = - omegal(l)*( ql(l)+qccl(l)+qiil(l)-ql(l+1)-qccl(l+1)-qiil(l+1) )/(pmid(l)-pmid(l+1))
          ttend(k) = - omegal(l)*( (tl(l)+gamaz(k)-fac_cond*(qccl(l)+qiil(l))-fac_fus*qiil(l)) - &
...
#ifdef CRMACCEL      
        if (use_crm_accel) then   !hparish modified to scale nstop for crm_acc.
          if (mod (real(nstop),(1.+crm_accel_factor)) .ne. 0 ) then
            write (0,*) "CRM acceleration unexpected exception:"
            write (0,*) "(1+crm_accel_factor) does not divide equally into nstop"
            write (0,*) "nstop = ", nstop
            write (0,*) "crm_accel_factor = ", crm_accel_factor
            call endrun('crm main')
          end if
          nstop = (dt_gl/dt)/(1.D0 + crm_accel_factor)
        end if
#endif
...
!       endif
        ...
        nstep_adv = 0 ! 5/5/2015, hparish added this line to restart nstep_adv for the new timestep. 
!                       For UM5 advetion scheme only. Will not clash with MPDATA.
        nstep = 0
        nprint = 1
        ncycle = 0
!        nrad = nstop/nrad0
...
!    collect tke related statistics in buoyancy() only on the last iteration - mwyant
     if(icyc.eq.ncycle) then 
	dostatis = .true.
     else
        dostatis = .false.
     end if
...
!---mhwangtest 
     ...
     utendout = utend       !hparish wants to output the u,v tendencies right befor they are used by forcing.
     vtendout = vtend       !utend and vtend are used to feed SAMUW following arrangements with Matt W.
...
!   if (latitude0 .gt. -24. .and. latitude0 .lt. -13 .and. longitude0 .gt. 260. .and. longitude0 .lt. 290.) then
     ! Near the VOCA region, change the GCM - CRM forcing from full HADV and VADV of Q,T to:
     ! JUST OMEGA PROFILE! Then full 3D T,q anomalies that develop and their associated vertical gradients
     ! convolve with omega to produce a domain-mean "vertical advection" that is a *function of the evolving
     ! turbulence field... i.e. it will change over time. And this should in theory give the turbulence
     ! dynamics more internal control (and less external constraint) on the developing inversion.

! <snip from SAM subsidence.f90, but updated to include l-indexed GCM variables (omega, pmid)
! and using omegadXdP in places of wsubdXdz> -- pritch.
! vars to define: k1,k2,l1,l2,t_tend(i,j,k),q_tend(i,j,k),dq
     do k=3,nzm-1
       l = plev-k+1
       if(omegal(l).ge.0) then
         k1 = k+1
         k2 = k
         l1 = l-1
         l2 = l
       ...
       do j=1,ny
         do i=1,nx
           t_tend(i,j,k) = -omegal(l)*(t(i,j,k1)-t(i,j,k2))/(pmid(l1)-pmid(l2))
           q_tend(i,j,k) = -omegal(l)*(micro_field(i,j,k1,index_water_vapor)-micro_field(i,j,k2,index_water_vapor))/(pmid(l1)-pmid(l2))
         end do
       end do
      ...
      !    water (i.e., vapor+cloud).
       do n = 1,nmicro_fields
         if(n.ne.index_water_vapor) then
           do j=1,ny
              do i=1,nx
                 dq = - omegal(l)*(micro_field(i,j,k1,n)-micro_field(i,j,k2,n))/(pmid(l1)-pmid(l2))
                 micro_field(i,j,k,n) = micro_field(i,j,k,n) + dtn*dq
!                 mklsadv(k,n) = mklsadv(k,n) + dq ! pritch suspects just diagnostic.
              end do
           end do
         end if
       end do
     ...
     do k=2,nzm-1
       ...
       do j=1,ny
         do i=1,nx
           ! Apply CRM-column dependent forcing (unlike standard GCM-CRM coupling, which applies a single Ttend profile to all CRM columns)
           ! So the following two lines replace regular forcing in crm_forcing.F90
           t(i,j,k) = t(i,j,k) + dtn * t_tend(i,j,k)
           micro_field(i,j,k,index_water_vapor) = max(0.,micro_field(i,j,k,index_water_vapor) &
                         + dtn * q_tend(i,j,k))
! WARNING there could be a leak of water from the system in the above line. May need to consider re-introducing it.
           t_vtend = t_vtend + t_tend(i,j,k) ! diag
           q_vtend = q_vtend + q_tend(i,j,k)
         end do
       end do
       t_vtend = t_vtend / float(nx*ny) ! diag
       q_vtend = q_vtend / float(nx*ny)
       ttend(k) = ttend(k) + t_vtend  ! diag
...
! Make sure to also add the surface flux component of the forcing:
     do k=1,2
       do j=1,ny
         do i=1,nx
   ...
   else  ! not in the attack region? Use the regular GCM-derived CRM-wide forcing profiles.
      call crm_forcing()
   end if
#else
!------------------------------------------------------------
...
!   	suppress turbulence near the upper boundary (spange):
     ...
     if(dodamping) then
#ifdef STRATOKILLER
       call damping(plev-klev_crmtop_gcm+1,stratokill_debug_flag) ! convert argument from GCM grid to CRM grid in the vertical.
#else
       call damping()
#endif
     endif
...
! Use Jones-Bretherton methodology to accelerate
! CRM horizontal mean evolution artificially.

        if (.not. accel_skipping ) then
          call accelerate_crm(crm_accel_factor,crm_accel_ceaseflag)
          if (crm_accel_ceaseflag) then
             accel_skipping = .true.
             write (6,*) 'PRITCH: HEY accel_skipping triggered @ lat,lon,nstep,icyc=',latitude0,longitude0,nstep,icyc
             write (6,*) 'PRITCH: HEY nstop increasd from,to',nstop,nstop+(nstop-nstep)*crm_accel_factor
             nstop = nstop + (nstop-nstep)*crm_accel_factor ! only can happen once.
          endif
        endif
      endif
#endif
...
!     if(lchnk.eq.3100 .and. icol.eq.1) then     ! hparish is tapping to some data for debugging. Not needed in normal operation.
!        l2norm_qcl = EUC_NORM ( REAL(qcl(1:nx,1:ny,1:nzm)), 1, nx, 1, ny, 1, nzm )
!!        l2norm_cl = EUC_NORM ( cloudliq(1:nx,1:ny,1:nzm), 1, nx, 1, ny, 1, nzm )
!        write(6, *) 'lch,i,l2n_qcl=',lchnk,icol,l2norm_qcl
!     endif
...
!------------------ !hparish outputs intra-crm stats
! -- pritch added for output at VOCA region of interest --
       if ( (latitude0 .gt. -20. .and. latitude0 .lt. -19.8 .and. longitude0 .gt. 274.9 .and. longitude0 .lt. 275.1) .or. &
          (latitude0 .gt. -20. .and. latitude0 .lt. -19.8 .and. longitude0 .gt. 284.9 .and. longitude0 .lt. 285.1) ) then
         ...
         do k=1,nzm
          ...
          do j=1,ny
           do i=1,nx
            crm_qc_inst(l)  = crm_qc_inst(l)  + qcl(i,j,k)
            crm_q_inst(l)   = crm_q_inst(l)   + micro_field(i,j,k,index_water_vapor)
            crm_t_inst(l)   = crm_t_inst(l)   + t(i,j,k)
            crm_tke_inst(l) = crm_tke_inst(l) + tke(i,j,k)
           end do
          end do
         end do

!         if (latitude0 .gt. -20. .and. latitude0 .lt. -19.8 .and. longitude0 .gt. 274.9 .and. longitude0 .lt. 275.1) then 
!          write (666,*) nstep, latitude0, longitude0
!          write (666,*) crm_qc_inst(:)  ! something that goes to/fro with GCM
!          write (666,*) crm_q_inst (:)
!          write (666,*) crm_t_inst (:)
!          write (666,*) crm_tke_inst (:)
!          write (666,*) crm_ww_inst (:)     ! something that does not.
!          write (666,*) z(:)     
!         endif

!         if (latitude0 .gt. -20. .and. latitude0 .lt. -19.8 .and. longitude0 .gt. 284.9 .and. longitude0 .lt. 285.1) then
!          write (777,*) nstep, latitude0, longitude0
!          write (777,*) crm_qc_inst(:)  ! something that goes to/fro with GCM
!          write (777,*) crm_q_inst (:)
!          write (777,*) crm_t_inst (:)
!          write (777,*) crm_tke_inst (:)
!          write (777,*) crm_ww_inst (:)     ! something that does not.
!          write (777,*) z(:)     
!         endif

       endif

!         if (latitude0 .gt. 81. .and. latitude0 .lt. 83. .and. longitude0 .gt. -1.0 .and. longitude0 .lt. 1.0) then
!          write (888,*) nstep, latitude0, longitude0
!          write (888,*) tabs
!          write (888,*) z(:)
!         endif

!         if (latitude0 .gt. 81. .and. latitude0 .lt. 83. .and. longitude0 .gt. 329.0 .and. longitude0 .lt. 331.0) then
!          write (999,*) nstep, latitude0, longitude0
!          write (999,*) tabs
!          write (999,*) z(:)
!         endif
!------------------
...
! HP and MSP removed the following lines to account for the double counting in cloud fraction normalization when crm_acc is ON.
!#ifdef CRMACCEL
!        cltot = cltot * (1.+crm_accel_factor)
!        clhgh = clhgh * (1.+crm_accel_factor)
!        clmed = clmed * (1.+crm_accel_factor)
!        cllow = cllow * (1.+crm_accel_factor)
!#endif
...
#ifdef SHORTCRM 
!      hparish is following bloss' suggestion hoping to fix the excessive stratospheric ice bias. 
!      Above the CRM top, move any GCM cloud liquid/ice to vapor.
!      The GCM does not have any microphysical processes going on at these heights,
!      so that it unrealistic to advect around any ice that would likely be lost
!      to sublimation if microphysical processes were enabled.

        qln(1:ptop-1) = ql(1:ptop-1) + qccl(1:ptop-1) + qiil(1:ptop-1)
        qccln(1:ptop-1)= 0.
        qiiln(1:ptop-1)= 0.
#endif
...
#ifndef SHORTCRM 
!       hparish is following bloss' suggestion hoping to fix the excessive stratospheric ice bias. 
!       Do not zero out moisture tendencies from the CRM in the top two levels.
!       Neglecting the CRM tendencies there tends to deposit cloud ice in the tropical lower
!       stratosphere, and without the microphysical processes in the CRM, this ice persists
!       and is advected through the stratosphere to the poles by the Brewer-Dobson circulation.
...
#ifdef SHORTCRM 
!       hparish is following bloss' suggestion hoping to fix the excessive stratospheric ice bias. 
!       include temperature tendency due to cloud liquid/cloud ice tendencies
!       at these top two CRM levels, so that the energy budget will be consistent.
!       In doing this, we are neglecting the advective tendencies from the CRM in those top
!       two levels.
        sltend(ptop:ptop+1) = - fac_cond*qcltend(ptop:ptop+1) &
                               - fac_sub*qiltend(ptop:ptop+1)
#endif
...
!  crt 11/26/18 added changes
        ...
        mc = mcup + mcdn + mcuup + mcudn
        crm_ww = crm_ww / float(nstop) * factor_xy ! mspritch,hparish
...
! test water conservtion problem
      do k=1, nzm
        ...
        if(abs(qtot(9)-qtot(1))/qtot(1).gt.1.0e-4) then
...
!          mkwsb(k,:) = mkwsb(k,:) * tmp1*rhow(k) * factor_xy/nstop     !kg/m3/s --> kg/m2/s
          mkwsb(k,:) = mkwsb(k,:) * tmp1*rhow(k) * factor_xy     !kg/m3/s --> kg/m2/s    !hparish removes averaging by nstop.
!          mkwle(k,:) = mkwle(k,:) * tmp2*rhow(k) * factor_xy/nstop     !kg/m3   --> kg/m2/s  
          mkwle(k,:) = mkwle(k,:) * tmp2*rhow(k) * factor_xy     !kg/m3   --> kg/m2/s    !hparish removes averaging by nstop.  
...
! qpsrc, qpevp, qpfall in M2005 are calculated in micro_flux. 
          ...
          l = plev-k+1
          flux_u(l) = (uwle(k) + uwsb(k))*tmp1*factor_xy/nstop     !nstop shoud be removed here.
          flux_v(l) = (vwle(k) + vwsb(k))*tmp1*factor_xy/nstop     !nstop shoud be removed here.
#ifdef sam1mom
