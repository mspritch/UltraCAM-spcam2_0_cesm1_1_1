...
public :: microp_aero_init, microp_aero_run, microp_aero_readnl
...
subroutine microp_aero_run ( &
   state, ptend, deltatin, pbuf, liqcldf, &
   icecldf, naai, naai_hom, npccn, rndst, &
   nacon)
   ...
   real(r8), intent(out) :: rndst(pcols,pver,4) ! radius of 4 dust bins for contact freezing
   real(r8), intent(out) :: nacon(pcols,pver,4) ! number in 4 dust bins for contact freezing
   ...
   do k = top_lev, pver
      ...
      call vqsatd_water(t(:,k), pmid(:,k), es, qs, gammas, ncol)
