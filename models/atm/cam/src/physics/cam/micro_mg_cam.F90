...
use physics_types,  only: physics_state, physics_ptend, physics_tend, physics_ptend_init, &
                          physics_state_copy, physics_ptend_copy, physics_tend_init, &
                          physics_update
use physics_buffer, only: physics_buffer_desc, pbuf_add_field, pbuf_times, &
                          pbuf_old_tim_idx, pbuf_get_index, dtype_r8, dtype_i4, pbuf_get_field, pbuf_set_field
use constituents,   only: cnst_add, cnst_get_ind, &
                          cnst_name, cnst_longname, sflxnam, apcnst, bpcnst

use gamma_function, only: gamma
use wv_saturation,  only: vqsatd_water, polysvp, epsqs 
use cldwat2m_macro, only: rhmini
use micro_mg,       only: micro_mg_init, micro_mg_tend
...
subroutine micro_mg_cam_readnl(nlfile)
  ...
  namelist /micro_mg_nl/ micro_mg_do_cldice, micro_mg_do_cldliq
...
subroutine micro_mg_cam_register
   ...
   call pbuf_add_field('CLDFSNOW ',  'physpkg',dtype_r8,(/pcols,pver,pbuf_times/), cldfsnow_idx) ! Cloud fraction for liquid drops + snow
...
subroutine micro_mg_cam_init(pbuf2d)
   ...
   call micro_mg_init( &
      ...
      rhoh2o, tmelt, latvap, latice, epsqs, &
      rhmini, gamma, errstring)
   if (errstring /= '') call endrun('micro_mg_cam_init: error return from micro_mg_init: '&
                                    //trim(errstring))
...
subroutine micro_mg_cam_tend(state, ptend, dtime, pbuf, cmeliq, &
   ...
   real(r8), intent(in) :: naai(pcols,pver)      ! ice nucleation number
   real(r8), intent(in) :: naai_hom(pcols,pver)  ! ice nucleation number (homogeneous)
   real(r8), intent(in) :: npccn(pcols,pver)     ! liquid activation number tendency
   ...
   logical :: sub_column = .false. ! True = configure microphysics for sub-columns 
   ...
   type(physics_tend)  :: tend_loc

   real(r8), pointer :: aist(:,:)         ! Physical ice stratus fraction
   real(r8), pointer :: alst(:,:)         ! Physical liquid stratus fraction
   ...
   ! variables for autconversion v accretion vertical averages
   ...
   call micro_mg_tend( &
      sub_column, pcols, pver, ncol, top_lev, dtime,   &
   ...
   if (errstring /= '') call endrun('micro_mg_tend: error: '//trim(errstring))
   ...
   if (rate1_cw2pr_st_idx > 0) then
      ...
      rate1ord_cw2pr_st(1:ncol,:) = rate1cld(1:ncol,:)
   end if
   ...
   ! the name 'cldwat' triggers special tests on cldwat and cldice in physics_update
   ptend%name = 'cldwat'
   ptend%ls           = .true.
   ptend%lq(1)        = .true.
   ...
   do k = 1, pver
      do i = 1, ncol
         ptend%s(i,k)          =  tlat(i,k)
         ptend%q(i,k,1)        = qvlat(i,k)
         ptend%q(i,k,ixcldliq) = qcten(i,k)
         ptend%q(i,k,ixcldice) = qiten(i,k)
         ptend%q(i,k,ixnumliq) = ncten(i,k)
         ptend%q(i,k,ixnumice) = niten(i,k)
         ...
         if ((.not. do_cldice) .and. (qiten(i,k) /= 0.0_r8)) then 
            call endrun("micro_mg_cam:ERROR - MG microphysics is configured not to prognose cloud ice, but micro_mg_tend has ice mass tendencies.")
         end if
         if ((.not. do_cldice) .and. (niten(i,k) /= 0.0_r8)) then 
            call endrun("micro_mg_cam:ERROR - MG microphysics is configured not to prognose cloud ice, but micro_mg_tend has ice number tendencies.")
         end if
         
         if ((.not. do_cldliq) .and. (qiten(i,k) /= 0.0_r8)) then 
            call endrun("micro_mg_cam:ERROR - MG microphysics is configured not to prognose cloud liquid, but micro_mg_tend has liquid mass tendencies.")
         end if
         if ((.not. do_cldliq) .and. (niten(i,k) /= 0.0_r8)) then 
            call endrun("micro_mg_cam:ERROR - MG microphysics is configured not to prognose cloud liquid, but micro_mg_tend has liquid number tendencies.")
         end if
      end do
   end do
   ...
   call physics_tend_init(tend_loc)
   call physics_update(state_loc, tend_loc, ptend_loc, dtime)
   ...
   if( conv_water_in_rad /= 0 ) then
      ...
      allcld_liq(:ncol,:) = state_loc%q(:ncol,:,ixcldliq)  ! Grid-ave all cloud liquid
      allcld_ice(:ncol,:) = state_loc%q(:ncol,:,ixcldice)  !           "        ice 
   end if
   ...
   do k = 1, pver
      do i = 1, ncol
         ...
         icinc(i,k)     = state_loc%q(i,k,ixnumice) / max(0.0001_r8,icecldf(i,k)) * state_loc%pmid(i,k) / (287.15_r8*state_loc%t(i,k))
         icwnc(i,k)     = state_loc%q(i,k,ixnumliq) / max(0.0001_r8,liqcldf(i,k)) * state_loc%pmid(i,k) / (287.15_r8*state_loc%t(i,k))
...
!        endif
   ...
   ! vars: vprco(i),vprao(i),racau(i),ccnt,acnt
   ...
   do i = 1,ncol
      ...
      acnt = 0
      do k = 1, pver
         if (prco(i,k) /= 0._r8) then
            vprco(i) = vprco(i) + prco(i,k)
            ccnt     = ccnt + 1
         end if
         if (prao(i,k) /= 0._r8) then
            vprao(i)=vprao(i) + prao(i,k)
            acnt = acnt + 1
         end if
      end do
      if(ccnt > 0) then
         vprco(i) = vprco(i)/ccnt
      end if
      if(acnt > 0) then
         vprao(i) = vprao(i)/acnt
         racau(i) = vprco(i)/vprao(i)
      end if
   end do
   ...
   do i = 1, ncol
      cdnumc(i) = 0._r8
      do k = 1, pver
         cdnumc(i) = cdnumc(i) + state_loc%q(i,k,ixnumliq)*state_loc%pdel(i,k)/gravit
      end do
   end do
