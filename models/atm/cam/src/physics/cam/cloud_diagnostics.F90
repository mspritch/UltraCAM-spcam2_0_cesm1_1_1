...
!===============================================================================
  subroutine cloud_diagnostics_register
    ...
    rk_clouds = microp_pgk .eq. 'RK'
    mg_clouds = microp_pgk .eq. 'MG'
...
!-----------------------------------------------------------------------
    ...
    call addfld ('ICLDTWP ',wpunits, pver, 'A','In-cloud cloud total water path (liquid and ice)',phys_decomp, sampling_seq=sampling_seq)
    call add_default ('ICLDTWP', 1, ' ')
    call add_default ('ICLDIWP', 1, ' ')

    call addfld ('GCLDLWP ',wpunits,pver, 'A','Grid-box cloud water path'             ,phys_decomp, sampling_seq=sampling_seq)
    call addfld ('TGCLDCWP',wpunits,1,    'A','Total grid-box cloud water path (liquid and ice)',phys_decomp, sampling_seq=sampling_seq)
    call addfld ('TGCLDLWP',wpunits,1,    'A','Total grid-box cloud liquid water path',phys_decomp, sampling_seq=sampling_seq)
    call addfld ('TGCLDIWP',wpunits,1,    'A','Total grid-box cloud ice water path'   ,phys_decomp, sampling_seq=sampling_seq)
