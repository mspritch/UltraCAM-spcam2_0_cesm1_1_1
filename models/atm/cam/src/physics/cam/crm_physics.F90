...
!--------------------------------------------------------------------------------------------------------
  ...
  use ppgrid,          only: pcols, pver, pverp
  use physics_buffer,  only: pbuf_times, pbuf_add_field, dtype_r8, pbuf_get_index
...
! local variables
  ...
  if (SPCAM_microp_scheme .eq. 'm2005') then
     call addfld ('SPNC    ','/kg   ',pver, 'A','Cloud water dropet number from CRM',phys_decomp)
     call addfld ('SPNI    ','/kg   ',pver, 'A','Cloud ice crystal number from CRM',phys_decomp)
     call addfld ('SPNS    ','/kg   ',pver, 'A','Snow particle number from CRM',phys_decomp)
     call addfld ('SPNG    ','/kg   ',pver, 'A','Graupel particle number from CRM',phys_decomp)
     call addfld ('SPNR    ','/kg   ',pver, 'A','Rain particle number from CRM',phys_decomp)
     ...
     call addfld ('CRM_FLIQ ','1      ',crm_nx*crm_ny*crm_nz, 'A','Frequency of Occurrence of Liquid',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_FICE ','1      ',crm_nx*crm_ny*crm_nz, 'A','Frequency of Occurrence of Ice',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_FRAIN','1      ',crm_nx*crm_ny*crm_nz, 'A','Frequency of Occurrence of Rain',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_FSNOW','1      ',crm_nx*crm_ny*crm_nz, 'A','Frequency of Occurrence of Snow',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_FGRAP','1      ',crm_nx*crm_ny*crm_nz, 'A','Frequency of Occurrence of Graupel',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_QS  ','kg/kg   ',crm_nx*crm_ny*crm_nz, 'A','Snow mixing ratio from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_QG  ','kg/kg   ',crm_nx*crm_ny*crm_nz, 'A','Graupel mixing ratio from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_QR  ','kg/kg   ',crm_nx*crm_ny*crm_nz, 'A','Rain mixing ratio from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))

     call addfld ('CRM_NC  ','/kg     ',crm_nx*crm_ny*crm_nz, 'A','Cloud water dropet number from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_NI  ','/kg     ',crm_nx*crm_ny*crm_nz, 'A','Cloud ice crystal number from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_NS  ','/kg     ',crm_nx*crm_ny*crm_nz, 'A','Snow particle number from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_NG  ','/kg     ',crm_nx*crm_ny*crm_nz, 'A','Graupel particle number from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_NR  ','/kg     ',crm_nx*crm_ny*crm_nz, 'A','Rain particle number from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))

! hm 7/26/11, add new output
! below is for *instantaneous* crm output
     call addfld ('CRM_AUT  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Autoconversion cloud waterfrom CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_ACC  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Accretion cloud water from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_EVPC ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Evaporation cloud water from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_EVPR ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Evaporation rain from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_MLT  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Melting ice snow graupel from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_SUB  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Sublimation ice snow graupel from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_DEP  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Deposition ice snow graupel from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_CON  ','/s     ',crm_nx*crm_ny*crm_nz, 'A','Condensation cloud water from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
! hm 8/31/11
! below is for *gcm-grid and time-step-avg* process output
     call addfld ('A_AUT  ','/s   ',pver, 'A','Avg autoconversion cloud water from CRM',phys_decomp)
     call addfld ('A_ACC  ','/s   ',pver, 'A','Avg accretion cloud water from CRM',phys_decomp)
     call addfld ('A_EVPC ','/s   ',pver, 'A','Avg evaporation cloud water from CRM',phys_decomp)
     call addfld ('A_EVPR ','/s   ',pver, 'A','Avg evaporation rain from CRM',phys_decomp)
     call addfld ('A_MLT  ','/s   ',pver, 'A','Avg melting ice snow graupel from CRM',phys_decomp)
     call addfld ('A_SUB  ','/s   ',pver, 'A','Avg sublimation ice snow graupel from CRM',phys_decomp)
     call addfld ('A_DEP  ','/s   ',pver, 'A','Avg deposition ice snow graupel from CRM',phys_decomp)
     call addfld ('A_CON  ','/s   ',pver, 'A','Avg condensation cloud water from CRM',phys_decomp)

     call addfld ('CRM_REL  ', 'micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale droplet effective radius', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_REI  ', 'micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale ice crystal effective radius', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_DEI  ', 'micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale Mitchell ice effective diameter', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_DES  ', 'micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale snow effective diameter', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_MU   ', 'micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale droplet size distribution shape parameter for radiation', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_LAMBDA','micrometers', crm_nx*crm_ny*crm_nz, 'A', 'cloud scale slope of droplet distribution for radiation', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_TAU  ', '1',           crm_nx*crm_ny*crm_nz, 'A', 'cloud scale cloud optical depth', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('CRM_WVAR' , 'm/s',         crm_nx*crm_ny*crm_nz, 'A','vertical velocity variance from CRM',phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))

     call addfld ('CRM_FSNT',    'unitless',  crm_nx*crm_ny,        'A', 'net TOA shortwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FSNTC',   'unitless',  crm_nx*crm_ny,        'A', 'net TOA clear-sky shortwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FSNS',    'unitless',  crm_nx*crm_ny,        'A', 'net surface shortwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FSNSC',   'unitless',  crm_nx*crm_ny,        'A', 'net surface clear-sky shortwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FLNT',    'unitless',  crm_nx*crm_ny,        'A', 'net TOA longwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FLNTC',   'unitless',  crm_nx*crm_ny,        'A', 'net TOA clear-sky longwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FLNS',    'unitless',  crm_nx*crm_ny,        'A', 'net surface longwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_FLNSC',   'unitless',  crm_nx*crm_ny,        'A', 'net surface clear-sky longwave fluxes at CRM grids', phys_decomp, &
                                        mdimnames=(/'crm_nx','crm_ny'/))

     call addfld ('CRM_AODVIS',  'unitless',  crm_nx*crm_ny,        'A', 'Aerosol optical depth at 550nm in CRM grids', phys_decomp, &
                                        flag_xyfill=.true., &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_AOD400',  'unitless',  crm_nx*crm_ny,        'A', 'Aerosol optical depth at 400nm in CRM grids', phys_decomp, flag_xyfill=.true., &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_AOD700',  'unitless',  crm_nx*crm_ny,        'A', 'Aerosol optical depth at 700nm in CRM grids', phys_decomp, flag_xyfill=.true., &
                                        mdimnames=(/'crm_nx','crm_ny'/))
     call addfld ('CRM_AODVISZ', 'unitless',  crm_nx*crm_ny*crm_nz, 'A', 'Aerosol optical depth at each layer at 500nm in CRM grids',&
                                        phys_decomp, flag_xyfill=.true., &
                                        mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
     call addfld ('AOD400', 'unitless', 1, 'A', 'Aerosol optical depth at 400nm', phys_decomp, flag_xyfill=.true.)
     call addfld ('AOD700', 'unitless', 1, 'A', 'Aerosol optical depth at 700nm', phys_decomp, flag_xyfill=.true.)
     call add_default ('AOD400',  1, ' ')
     call add_default ('AOD700',  1, ' ')
   endif
#ifdef CLUBB_CRM
  call addfld ('UP2     ','m^2/s^2',   crm_nx*crm_ny*(crm_nz+1), 'A',   'u prime ^2 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('VP2     ','m^2/s^2',   crm_nx*crm_ny*(crm_nz+1), 'A',   'v prime ^2 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('WPRTP   ','mkg/skg',   crm_nx*crm_ny*(crm_nz+1), 'A',   'w prime * rt prime from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('WPTHLP  ','mK/s',      crm_nx*crm_ny*(crm_nz+1), 'A',   'w prime * th_l prime from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('WP2     ','m^2/s^2',   crm_nx*crm_ny*(crm_nz+1), 'A',   'w prime ^2 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('WP3     ','m^3/s^3',   crm_nx*crm_ny*(crm_nz+1), 'A',   'w prime ^3 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('RTP2    ','(kg/kg)2',  crm_nx*crm_ny*(crm_nz+1), 'A',   ' r_t prime ^2 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('THLP2   ','K^2',       crm_nx*crm_ny*(crm_nz+1), 'A',   ' th_l_prime ^2 from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('RTPTHLP ','kgK/kg',    crm_nx*crm_ny*(crm_nz+1), 'A',   ' r_t prime * th_l prime  from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('UPWP    ','m^2/s^2',   crm_nx*crm_ny*(crm_nz+1), 'A',   'u prime * w prime from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('VPWP    ','m^2/s^2',   crm_nx*crm_ny*(crm_nz+1), 'A',   'v prime * w prime from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/))
  call addfld ('CRM_CLD ','fraction',  crm_nx*crm_ny*(crm_nz+1), 'A',   'cloud fraction from clubb',phys_decomp, &
                                     mdimnames=(/'crm_nx','crm_ny','crm_nz'/) )
#endif

#ifdef ECPP
  if (use_ECPP) then
     call addfld ('ABND    ', 'fraction', pverp*NCLASS_CL*ncls_ecpp_in*NCLASS_PR, 'A', &
                  'cloud fraction for each sub-sub class for full time period at layer boundary', phys_decomp, &
                  mdimnames=(/'pverp       ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('ABND_TF ', 'fraction', pverp*NCLASS_CL*ncls_ecpp_in*NCLASS_PR, 'A', &
                  'cloud fraction for each sub-sub class for end-portion of time period at layer boundary', phys_decomp, &
                  mdimnames=(/'pverp       ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('MASFBND ', 'kg/m2/s',  pverp*NCLASS_CL*ncls_ecpp_in*NCLASS_PR, 'A', &
                  'sub-class vertical mass flux (kg/m2/s) at layer boundary', phys_decomp, &
                  mdimnames=(/'pverp       ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('ACEN    ', 'fraction', pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud fraction for each sub-sub class for full time period at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('ACEN_TF ', 'fraction', pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud fraction for each sub-sub class for end-portion of time period at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('RHCEN   ', 'fraction', pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'relative humidity for each sub-sub calss at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('QCCEN   ', 'kg/kg',    pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud water for each sub-sub class at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/)) 
     call addfld ('QICEN   ', 'kg/kg',    pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud ice for each sub-sub class at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('QSINK_AFCEN', '/s',    pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud water loss rate from precip. using cloud water after precip. for each sub-sub class at layer center', &
                  phys_decomp, mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('QSINK_BFCEN', '/s',    pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud water loss rate from precip. using cloud water before precip. for each sub-sub class at layer center', &
                  phys_decomp, mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('QSINK_AVGCEN', '/s',   pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'cloud water loss rate from precip. using averaged cloud water and precip. rate for each sub-sub class at layer center', &
                  phys_decomp, mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('PRAINCEN', 'kg/kg/s',  pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  ' cloud water loss rate from precipitation (kg/kg/s) for each sub-sub class at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('PRECRCEN', 'kg/m2/s',  pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'liquid (rain) precipitation rate for each sub-sub class at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('PRECSCEN', 'kg/m2/s',  pver*NCLASS_CL*ncls_ecpp_in*NCLASS_PR,  'A', &
                  'solid (snow, graupel,...) precipitation rate for each sub-sub class at layer center', phys_decomp, &
                  mdimnames=(/'pver        ','NCLASS_CL   ','ncls_ecpp_in','NCLASS_PR   '/))
     call addfld ('WUPTHRES', 'm/s',        pverp, 'A', 'vertical velocity threshold for updraft',  phys_decomp)
     call addfld ('WDNTHRES', 'm/s',        pverp, 'A', 'vertical velocity threshold for dndraft',  phys_decomp)
     call addfld ('WWQUI_CEN', 'm2/s2',     pver,  'A', 'vertical velocity variance in the quiescent class, layer center',  phys_decomp)
     call addfld ('WWQUI_CLD_CEN', 'm2/s2', pver,  'A', &
                  'vertical velocity variance in the cloudy quiescent class, layer center',  phys_decomp)
     call addfld ('WWQUI_BND', 'm2/s2',     pverp, 'A', &
                  'vertical velocity variance in the quiescent class, layer boundary',  phys_decomp)
     call addfld ('WWQUI_CLD_BND', 'm2/s2', pverp, 'A', &
                  'vertical velocity variance in the cloudy quiescent class, layer boundary',  phys_decomp)
  endif
#endif

  call addfld ('MU_CRM   ', 'Pa/s', pver, 'A', 'mass flux up from CRM', phys_decomp)
  call addfld ('MD_CRM   ', 'Pa/s', pver, 'A', 'mass flux down from CRM', phys_decomp)
  call addfld ('DU_CRM   ', '/s', pver, 'A', 'detrainment from updraft from CRM', phys_decomp)
  call addfld ('EU_CRM   ', '/s', pver, 'A', 'entraiment rate from updraft', phys_decomp)
  call addfld ('ED_CRM   ', '/s', pver, 'A', 'entraiment rate from downdraft', phys_decomp)

  do m=1, pcnst 
    if(cnst_name(m) == 'DMS') then 
       call addfld(trim(cnst_name(m))//'CONV', 'kg/kg/s', pver, 'A', &
               trim(cnst_name(m))//' tendency from ZM convection', phys_decomp ) 
    end if
    ...
    call addfld ('SPQRL   ',  'K/s', pver, 'A', 'long-wave heating rate', phys_decomp)
    call addfld ('SPQRS   ',  'K/s', pver, 'A', 'short-wave heating rate', phys_decomp)

    call addfld ('LENGC     ','m       ',pverp, 'A', 'Mixing length scale for the calcuation of vertical difusivity', phys_decomp)

    call addfld ('SPKVH     ','m2/s    ',pverp, 'A','Vertical diffusivity used in dropmixnuc in the MMF call',phys_decomp)
    call addfld ('SPWTKE    ','m/s     ',pver,  'A','Standard deviation of updraft velocity',phys_decomp)
    call addfld ('SPLCLOUD  ','        ',pver,  'A','Liquid cloud fraction',phys_decomp)
    ...
    call addfld ('MICVARVTICE','',1,'A','vtice parameter',phys_decomp)
    call addfld ('MICVARTBGMIN','',1,'A','tbgmin parameter',phys_decomp)
    call addfld ('MICVARTBGMAX','',1,'A','tbgmax parameter',phys_decomp)
#endif
...
!-------------------------------------------------------------------------------------------
   use ppgrid
   use physics_buffer,  only: physics_buffer_desc, pbuf_old_tim_idx, pbuf_get_index, pbuf_times, pbuf_get_field, pbuf_set_field
...
#ifdef MICRO_VARY
   real(r8) microvary_qci0(pcols),microvary_qcw0(pcols),microvary_vtice(pcols),microvary_tbgmin(pcols),microvary_tbgmax(pcols)
#endif
...
! CRM column radiation stuff:
   ...
   real(r8) bflxout (pcols)  ! surface buoyancy flux (Km/s) for output    !hparish added this for output purpose only.
...
!---mhwangtest
   ...
   real(r8) tmp1 ! hparish: auxiliary variable to help feed the fluxes to CRM right before the call to CRM. 
   ...
   real(r8) :: rlon(pcols),rlat(pcols)
#ifdef CRMTRANSLATE
   real(r8) :: u_remove,v_remove,mass_sum
#endif
#ifdef STRATOKILLER
    integer :: klev_crmtop
...
! at layer boundary
   ...
   if(is_first_step())then
      ...
      enddo
      do i = 1,ncol
       if (rlat(i) .gt. -20. .and. rlat(i) .lt. -19.8 .and. rlon(i) .gt. 274.9 .and. rlon(i) .lt. 275.1) then                                  
!         open (UNIT=666, STATUS='NEW',FILE='LES_gory_details_666.txt',ACCESS='sequential')
       endif
       if (rlat(i) .gt. -20. .and. rlat(i) .lt. -19.8 .and. rlon(i) .gt. 284.9 .and. rlon(i) .lt. 285.1) then                                  
!         open (UNIT=777, STATUS='NEW',FILE='LES_gory_details_777.txt',ACCESS='sequential')
       endif
       if (rlat(i) .gt. 81.0 .and. rlat(i) .lt. 83.0 .and. rlon(i) .gt. -1.0 .and. rlon(i) .lt. 1.0) then   !polar ocean point
!         open (UNIT=888, STATUS='NEW',FILE='LES_gory_details_888.txt',ACCESS='sequential')
       endif
       if (rlat(i) .gt. 81.0 .and. rlat(i) .lt. 83.0 .and. rlon(i) .gt. 329.0 .and. rlon(i) .lt. 331.0) then   !polar land point
!         open (UNIT=999, STATUS='NEW',FILE='LES_gory_details_999.txt',ACCESS='sequential')
       endif
      end do
   endif
...
!      call check_energy_timestep_init(state, tend, pbuf)
    ...
    mass_sum = 0.
         ...
         do k=1,crm_nz
               m = pver-k+1
! INSERT calculate the horizontal wind at the inversion height.
! OR as a proxy to get started the 850-950 hPa average wind speed
! IF within desired region of interference, 
! Subtract that velocity from the initialized CRM velocity field.
#ifdef SP_DIR_NS
...
#ifdef CRMTRANSLATE
                if (abs(rlat(i)) .le. 35. .and. cam_in%landfrac(i) .eq. 0. .and. state%pmid(i,m)/100. .ge. 850. .and. state%pmid(i,m)/100. .le. 900.) then
                  u_remove = u_remove +state%u(i,m)*state%pdel(i,m)
                  v_remove = v_remove +state%v(i,m)*state%pdel(i,m)
                  mass_sum = mass_sum + state%pdel(i,m)
                endif
           end do
           if (abs(rlat(i)) .le. 35. .and. cam_in%landfrac(i) .eq. 0. .and. mass_sum .gt. 0.) then ! some grid points have no model layers in this range.
             u_remove = u_remove/mass_sum ! if we don't avoid them NaN creeps in here.
             v_remove = v_remove/mass_sum
 !          write (6,*) 'HEY u,v to remove are:',u_remove,v_remove
             do k=1,crm_nz
#ifdef SP_DIR_NS
               crm_u(i,:,:,k) = crm_u(i,:,:,k) - v_remove
               crm_v(i,:,:,k) = crm_v(i,:,:,k) - u_remove
#else
               crm_u(i,:,:,k) = crm_u(i,:,:,k) - u_remove
               crm_v(i,:,:,k) = crm_v(i,:,:,k) - v_remove
#endif
             end do
           endif
           do k=1,crm_nz
...
#ifdef SPFLUXBYPASS
  ! pritch -- apply surface flux perturbations to lowest level DSE, constituents here,
  ! instead of in vertical diffusion routine. To avoid exposiing dycore to a
  ! bottom-centric physics tendency profile component, and to allow SP to do the
  ! job of diffusing the flux infusion.
    ptend%lu = .false.
    ptend%lv = .false.
    ptend%ls = .true.
    ptend%lq = .false.
...
! total cloud water and total water vapor, and energy
       ...
       if (rlat(i) .gt. -72. .and. rlat(i) .lt. -68. .and. rlon(i) .gt. 103. .and. rlon(i) .lt. 107.) then                                                                                                                                                                 
          write (6,*) 'HEY YO klev_crmtop=',klev_crmtop,'p=',state%pmid(i,klev_crmtop)
       endif
#endif
...
!            , tropLev(i) &
             ...
             , microvary_vtice(i),microvary_tbgmin(i),microvary_tbgmax(i) &
#endif
             )
...
!HP       write(6, *) 'HP flag 1: crm is called. Inside subroutine: crm_physics_tend'
...
!      print*,'precc:',minval(precc(:ncol))*86400000.,maxval(precc(:ncol))*86400000.
       ...
       call outfld('CRM_QRAD ',crm_qrad(:, :, :, :)   ,pcols   ,lchnk   )   !hparish outputs crm_qrad right before it gets modified. For single column SAM forcing.
       call outfld('BFLX     ',bflxout                ,pcols   ,lchnk   )   !hparish outputs bflx. For single column SAM forcing.
...
!        print*,'ptend%s-qrad:',minval(ftem(:ncol,:))*86400.,maxval(ftem(:ncol,:))*86400.
       ...
       call outfld('SPWW    ',spww           ,pcols   ,lchnk   ) !  mspritch, hparish
       call outfld('SPBUOYA', spbuoya        ,pcols   ,lchnk    ) ! mwyant
       ...
       call outfld('CRM_UTEND',crm_utend     ,pcols   ,lchnk   )     !hparish outputted to compare against SAMUW.
       call outfld('CRM_VTEND',crm_vtend     ,pcols   ,lchnk   )     !hparish outputted to compare against SAMUW.
       ...
       call outfld('MICVARVTICE',microvary_vtice,pcols,lchnk)
       call outfld('MICVARTBGMIN',microvary_tbgmin,pcols,lchnk)
       call outfld('MICVARTBGMAX',microvary_tbgmax,pcols,lchnk)
#endif
...
! to check water conservations
             if(abs((qtot(i,2)+(precc(i)+precl(i))*1000*ztodt)-qtot(i,1))/qtot(i,1).gt.1.0e-4) then 
