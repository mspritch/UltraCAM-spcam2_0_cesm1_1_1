...
! When ECPP is defined, wet deposition is done in ECPP, so ptend%q is not upated here. 
!
...
! When ECPP is defined, wet deposition is calcualted in ECPP, so qqcw is not updated here.
!
             if (use_SPCAM .and. use_ECPP) then
                fldcw(1:ncol,:) = fldcw(1:ncol,:)
             else
