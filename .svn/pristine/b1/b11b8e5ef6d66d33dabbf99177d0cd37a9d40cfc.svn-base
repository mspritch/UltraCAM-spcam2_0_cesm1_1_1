...
! 
! May, 2009, Minghuai, Wang (Minghuai.Wang@pnl.gov),    MMF cam
!         The superparameterization treatment in cam3.5 (cam3.5+sam6.7.5) from Marat 
!      Khairoutdinov (mkhairoutdin@ms.cc.sunysb.edu) is adopted into this version of 
!      cam (4.0?). Short wave and long wave radiation codes are called for every column 
...
!      of the CRM domain loop (last_column=.true.). Gas-phase consitituents and aerosol optical
!      properties are only read at the beginning of CRM domain loop (first_column=.true.)
!      The main difference between cam3.5 and cam4: radcswmx is called three times in 
!      cam3.5, but is only called once in this versio of cam (4.0?).
!      This is the camrt radiation package used in cam4.0. For the rrtmg raidation package, 
!      the superparameterization treatment is also added into the radiation.F90. 
