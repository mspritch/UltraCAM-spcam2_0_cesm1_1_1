...
if [ -f ${CAM_TESTDIR}/${test_name}/TestStatus ]; then
    if grep -c PASS ${CAM_TESTDIR}/${test_name}/TestStatus > /dev/null; then
        echo "TNE_ccsm.sh: ccsm equivalent run test has already passed; results are in "
	...
	if [ $JOBID = $prev_jobid ]; then
            echo "TNE_ccsm.sh: ccsm equivalent run test has already failed for this job - will not reattempt; "
	    echo "        results are in: ${CAM_TESTDIR}/${test_name}" 
	    exit 2
	else
	    echo "TNE_ccsm.sh: this ccsm equivalent run test failed under job ${prev_jobid} - moving those results to "
...
${CAM_SCRIPTDIR}/TSM_ccsm.sh $1 $2 $4
rc=$?
if [ $rc -ne 0 ]; then
...
${CAM_SCRIPTDIR}/TSM_ccsm.sh $1 $3 $4
rc=$?
if [ $rc -ne 0 ]; then
