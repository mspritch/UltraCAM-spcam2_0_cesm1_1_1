...
use vars, only: pres, rho, dt, dtn, w, tke, tk, t, tlatqi, condavg_mask, &
...
real(8), allocatable, dimension(:,:,:) :: tauliq, tauice, cloudliq
...
#ifndef CRM
!bloss/qt: don't need to do initial saturation adjustment.
...
!!$        qc0(k) = max(0.,q0(k)-qsatw(tabs0(k),pres(k)))
!!$        qv0(k) = q0(k) - qc0(k)
!!$
...
!!$     end do
     ...
     do k = 1,nzm
        micro_field(:,:,k,iqv) = q0(k) !bloss/qt: changed from qv0
!bloss/qt        if(qc0(k).gt.0.) then
!bloss/qt           micro_field(:,:,k,iqcl) = qc0(k)
!bloss/qt           if(dopredictNc) micro_field(:,:,k,incl) = 1.e6*Nc0/rho(k) ! convert to mixing ratio
!bloss/qt        end if
        tabs(:,:,k) = tabs0(k)
     end do
#endif  ! CRM
...
#ifdef CLUBB_CRM
use crm_grid, only: doclubb, doclubb_sfc_fluxes, docam_sfc_fluxes
#endif
...
if ( doclubb .and. (doclubb_sfc_fluxes.or.docam_sfc_fluxes) ) then
  ...
  fluxtmk(:,:,index_water_vapor) = 0.0 ! top of domain qv flux
else
...
use crm_grid, only: z, zi

#ifdef CRM
use vars, only: docloud, t, gamaz, precsfc, precssfc, precflux, qpfall, tlat, prec_xy, &
#else
...
! Notes: tke has to be already prognsotic or diagnostic.
        tmpwsub = sqrt(tke(i,j,:)/3.)  ! diagnosed tmpwsub from tke
...
#else /* Old code */
!      tmpwsub = 0.                                                     !hparish zeroed tmpwsub to follow SAMUW.
! diagnose tmpwsub from tke.
! Notes: tke has to be already prognsotic or diagnostic.
      tmpwsub = sqrt(tke(i,j,:)/3.)  ! diagnosed tmpwsub from tke
! diagnose tmpwsub from tk
!      tmpwsub = sqrt(2*3.141593)*tk(i,j,:)/(dz*adz(:))  ! from Ghan et al. (1997, JGR).
      ...
      if ( doclubb ) then
        tmpqcl  = qcl(i,j,:) ! Liquid updated by CLUBB just prior to this
...
! hm 7/26/11, new output
          ...
          stendqr,stendqci,stendqs,stendqcl,real( cloud_frac(i,j,2:nz) )  & ! cloud_frac added by dschanen UWM
#ifdef ECPP
...
! divided by dt in crm.F90 to get mean rates
     ...
     ! update microphysical quantities in this grid column
      if(doprecip) then
         ...
         prec_xy(i,j) = prec_xy(i,j) + sfcpcp/dz   !hparish removed dtn from this line following SAM's bug fixes. See the next line for orignal.
...
subroutine satadj_liquid(nzm,tabs,qt,qc,pres)
  ...
  !  This routine perfomms a saturation adjustment for
  ...
  do k = 1,nzm
    ...
    if (qt(k).gt.qsat1) then
      ...
      !if(niter.gt.maxiter-1) write(*,*) 'Reached iteration limit in satadj_liquid'!BSINGH - commented out based on YS code
