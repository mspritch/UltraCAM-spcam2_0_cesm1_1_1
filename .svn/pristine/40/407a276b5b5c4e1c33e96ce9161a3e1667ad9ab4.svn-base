...
if(flag.eq.2) then
 ...
 if(dosgs.and..not.dosmagor.or.doscalar) &
     call bound_exchange(tke,dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,3+NADVS,3+NADVS,3+NADVS,3+NADVS,5)
 ...
 do i = 1,nmicro_fields
    if(   i.eq.index_water_vapor             &
     ...
     call bound_exchange(micro_field(:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,3+NADVS,3+NADVS,3+NADVS,3+NADVS,7+i)
 end do
 if(dotracers) then
   do i=1,ntracers
     call bound_exchange(tracer(:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,3+NADVS,3+NADVS,3+NADVS,3+NADVS,7+nmicro_fields+i)
   end do
 end if
