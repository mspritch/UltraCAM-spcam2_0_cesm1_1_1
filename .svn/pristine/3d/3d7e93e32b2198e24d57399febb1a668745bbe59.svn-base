...
subroutine progseasalts_emis_intr(state, ptend, dt, cam_in)
...
! the scaling factor for sea salt emissions (1.35 is used in the official release of CAM5)
! Note: the scaling factor for sea salt is used to multiply the fluxes, while the 
!       the scaling factor for dust is used to divide the fluxes. 

!  scaling factor for CAM5.1
        fact = 1.35_r8 
!  scaling factor at 1.9x2.5 resolution of the MMF run, without clubb
!       fact = 1.00_r8
...
!      sflx(:ncol)=sflx(:ncol)+cflx(:ncol,mm)
  ...
  subroutine ProgseasaltsDryDep(ncol,t,pmid,ram1,fv,vlc_dry,vlc_trb,vlc_grv,state)
    ...
    call aqsat(state%t,state%pmid,es,qs,pcols,ncol,pver,1,pver)
